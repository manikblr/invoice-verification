/**
 * MSC Industrial Supply Vendor Adapter
 */
...
export async function fetchVendorItems(opts: FetchOptions): Promise<VendorItem[]> {
  ...
  for (const query of opts.queries) {
    ...
    try {
      console.log(`  🔍 MSC: "${query}"`)
      
      const searchUrl = `https://www.mscdirect.com/browse/Search?q=${encodeURIComponent(query)}`
      ...
      $('.product-tile').each((i, elem) => {
        ...
        const sku = $elem.find('.product-number').text().trim()
        ...
        items.push({
          ...
          price,
          sku: sku || undefined,
          url: `https://www.mscdirect.com/product/${sku}`,
          line: opts.line
        })
      })
